...
while [ $j -lt ${#filesTotal[@]} ]; do
	...
	$inFile2' -S $outDir/$uniqueID/$uniqueID.sam"
	...
	samtools_line="samtools view -hbuS $outDir/$uniqueID/$uniqueID.sam | samtools sort - \
	'$outDir/$uniqueID/$uniqueID'"
	...
	samtools_view_line="samtools view -hf 4 '$outDir/$uniqueID/$uniqueID.bam' \
	| samtools view -Sb - > '$outDir/$uniqueID/unmapped_$uniqueID.bam'"
	...
	split_line="python $circDir/unmapped2anchors.py $outDir/$uniqueID/unmapped_$uniqueID.bam \
	| gzip > $outDir/$uniqueID/$uniqueID_anchors.qfa.gz"
	
	echo -e
	echo This is the split_line:
	echo $split_line

	#qsub -N spl$uniqueID -hold_jid stv$uniqueID -wd $logDir -b y -j y -P GenomeInformatics \
	...
	-U $outDir/$uniqueID/$uniqueID_anchors.qfa.gz -S $outDir/$uniqueID/$uniqueID_anchors.sam"
	...
	cat_line="cat $outDir/$uniqueID/$uniqueID_anchors.sam | python $circDir/find_circ.py -G \
	$GENOME -p $uniqueID -s $outDir/$uniqueID/sites.log"
	...
	#smp $small_numcores -V cat_line

	#filter the output:
	grep_line="grep circ '$outDir/$uniqueID/sites.bed'| grep -v chrM | python \
	$circDir/sum.py -2,3 | python $circDir/scorethresh.py -16 1 | python \
	$circDir/scorethresh.py -15 2 | python $circDir/scorethresh.py -14 2 | python \		$circDir/scorethresh.py 7 2 | python $circDir/scorethresh.py 8,9 35 | python \
	$circDir/scorethresh.py -17 100000 > $outDir/$uniqueID/circ_candidates.bed"
